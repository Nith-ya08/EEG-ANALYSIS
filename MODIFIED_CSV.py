        ...
        for line in lines_iter:
            if line.startswith("File Name:"):
                ...
                if num_seizures > 0:
                    ...
                    if file_name:
                        data.append([file_name, start_time, end_time, num_seizures, seizures_str, seizure_durations_str,
                                     average_seizure_duration])
                    else:
...
if all_data:
    df = pd.DataFrame(all_data, columns=['File Name', 'Start Time', 'End Time', 'Number of Seizures', 'Seizure Times',
                                         'Seizure Durations', 'Average Seizure Duration'])
    ...
    total_avg_seizure_duration_seconds = df['Average Seizure Duration Seconds'].sum() / \
                                         df[df['Number of Seizures'] > 0].shape[0]
